...
void Tasks::Init() {
    ...
    if (err = rt_sem_create(&sem_bloquant, NULL, 0, S_FIFO)) {
        cerr << "Error semaphore create: " << strerror(-err) << endl << flush;
        exit(EXIT_FAILURE);
    }
    ...
    if (err = rt_task_create(&th_findArena, "th_findArena", 0, PRIORITY_TOPENCAMERA, 0)) { //Peut etre faudrait-il changer une priorité?
        cerr << "Error task create: " << strerror(-err) << endl << flush;
        exit(EXIT_FAILURE);
    }
...
void Tasks::Join() {
    ...
    stopRobot = 0; //Le atomic int qui permet d'arreter le robot en cas de perte de communication est initialisé à 0
    ...
    while (1) {
        if (stopRobot == 0){
            ...
            if (msgRcv->CompareID(MESSAGE_MONITOR_LOST)) {
                ...
                stopRobot = 1;
                cout << "Communication with Monitor was lost" << endl << flush;
                rt_mutex_acquire(&mutex_monitor, TM_INFINITE);
                monitor.Close(); 
                rt_mutex_release(&mutex_monitor);
                ...
                robot.Close();
                //Pour tout relancer, il faut broadcast sem_bloquant après avoir remis stopRobot à 0
            ...
            cout << "Monitor Lost and robot stopped... Restart everything" << endl << flush;
            robot.Close();
        }
    }
}
...
void Tasks::MoveTask(void *arg) {
    ...
    while (1) {
        ...
        if (stopRobot == 0){
            ...
            if (rs == 1) {
                ...
                if ((msgSend2->GetID() == MESSAGE_ANSWER_COM_ERROR ) || (msgSend2->GetID() == MESSAGE_ANSWER_ROBOT_TIMEOUT)) {
                    ...
                    if (counter > 3)
                    {
                        ...
                        cout << "*******************************************************" <<endl; 
                        cout << "*******************************************************" <<endl; 
                        cout << "Perte de communication entre le robot et le superviseur" << endl; 
                        cout << "*******************************************************" <<endl; 
                        cout << "*******************************************************"; 
                        robot.Close(); 
                        rt_mutex_acquire(&mutex_robotStarted, TM_INFINITE);
            ...
            //Ajouter un semaphore bloquant
            rt_sem_p(&sem_bloquant,TM_INFINITE);
        }
        cout << endl << flush;
    }
}
    ...
    while(1){
        ...
        if (stopRobot == 0){
            if ((findingArena == 0) && (sendingPosition == 0) && (arenaInStock == 0)){
                ...
                rt_mutex_release(&mutex_camera);
            }else if((findingArena == 1) && (sendingPosition == 0) && (arenaInStock == 0)){

                cout << "Cond2" << endl << flush;
                ...
                rt_mutex_release(&mutex_camera);
            }else if((sendingPosition == 1) && (findingArena == 0) && (arenaInStock == 1)){
    ...
    while(1)
    {
        ...
        if(camera.IsOpen()){
            ...
            if (arena == nullptr){
                ...
                msgSend = new Message(MESSAGE_ANSWER_NACK);
                WriteInQueue(&q_messageToMon, msgSend); //Message will be deleted by sendToMon
                cout << "arena is nullptr" << endl << flush;
            }else{
                img->DrawArena(*arena);
                //Send image to monitor
                MessageImg * msgImg = new MessageImg(MESSAGE_CAM_IMAGE, img);
        ...
        cout << "findingArena=0" << endl << flush;
